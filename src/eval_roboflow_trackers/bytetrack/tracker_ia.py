...
from trackers.utils.state_representations import BaseStateEstimator, XYXYStateEstimator

from .tracklet_ia import ByteTrackTrackletIa


class ByteTrackTrackerIa(BaseTracker):
    ...
    def __init__(
        ...
        self.tracks: list[ByteTrackTrackletIa] = []
    ...
    def update(
        ...
        high_id = detection_id[high_indices]
        low_boxes = detection_boxes[low_indices]
        low_id = detection_id[low_indices]

        # Step 1: associate high-confidence detections to all tracks
        predicted_boxes = (
            np.array([t.get_state_bbox() for t in self.tracks]) if self.tracks else np.empty((0, 4))
        )
        ...
        for track in self.tracks:
            track.upd_id = UPD_ID_LOOSE

        for row, col in matched:
            track = self.tracks[row]
            track.update(high_boxes[col])
            track.upd_id = high_id[col]
            if (
        ...
        remaining_boxes = (
            predicted_boxes[unmatched_tracks] if unmatched_tracks else np.empty((0, 4))
        )
    ...
    def _get_associated_indices(
        self, similarity_matrix: np.ndarray, min_similarity_thresh: float
    ...
    def _spawn_new_tracks(
        ...
        out_det_indices: list[int],
        out_tracker_ids: list[int],
        detection_id: np.ndarray[tuple[int], np.dtype[np.int64]],
    ) -> None:
